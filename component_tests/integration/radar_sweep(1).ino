...
#define TRIG_PIN 6
#define ECHO_PIN 7
#define MAX_DISTANCE 15  // max distance in cm
...
// Maximum length of sweep line on the OLED
...
void drawRadar(int currentAngle, int currentDistance) {
  ...
  // Angle lines (static reference lines)
  ...
  // Sweep line length depends on detected object
  float rad = currentAngle * 3.14159 / 180;
  int lineLength = SWEEP_RADIUS;

  if(currentDistance > 0) {
    lineLength = map(currentDistance, 0, MAX_DISTANCE, 0, SWEEP_RADIUS);
  }
  ...
  // Object blip (tiny circle) at detected distance
  if(currentDistance > 0) {
    int blip = lineLength; // same as line end
    display.fillCircle(centerX - blip * cos(rad), centerY - blip * sin(rad), 2, WHITE);
  }

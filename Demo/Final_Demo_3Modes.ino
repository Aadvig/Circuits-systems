...
void drawMode2Graph(long currentDistance) {
  ...
  // threshold marker
  int threshY = map(thresholdDistance, 0, MAX_DISTANCE, xAxisY, graphY);
  display.drawLine(graphX, threshY, graphX + graphW - 1, threshY, WHITE);

  // plot
  for (int x = 1; x < GRAPH_SAMPLES; x++) {
    int d1 = constrain(distanceHistory[x - 1], 0, MAX_DISTANCE);
    int d2 = constrain(distanceHistory[x], 0, MAX_DISTANCE);

    int y1 = map(d1, 0, MAX_DISTANCE, xAxisY, graphY);
    int y2 = map(d2, 0, MAX_DISTANCE, xAxisY, graphY);
  ...
  display.print("B-SCAN");
  display.setCursor(0, 56);
  display.print("Distance: ");
  display.print(currentDistance);
  display.print(" cm");
  float totalSeconds = (GRAPH_SAMPLES * (getSweepDelay() + 10)) / 1000.0; // approx of total time
  display.setCursor(80, 56);
  // display.print("Time:");
  // display.print(totalSeconds, 1);
  // display.print("s");
...
void setup() {
  ...
  for (int i = 0; i < GRAPH_SAMPLES; i++) {
    distanceHistory[i] = 0; // set dist history to all zeros
  }
...
if (currentTouchState == HIGH && lastTouchState == LOW) {
  currentMode = (DisplayMode)((currentMode + 1) % 4);  // radar -> graph -> manual -> radar
  hapticFeedback();
